...
class SensorRecordControllerTest {
	...
	private RecordQueryRequestModel queryRequest;
	...
	public void queryRecordsControllerTest() throws Exception {
		...
		testQueryRequest = mapper.writeValueAsString(buildRecordQueryRequestModel());
		when(recordService.queryRecords(buildRecordQueryRequestModel())).thenReturn(new RecordQueryResponseModel());
	...
	public RecordQueryRequestModel buildRecordQueryRequestModel() {
		queryRequest = new RecordQueryRequestModel();
		Set<Long> ids = new HashSet<>();
		ids.add(1l);
		Set<String> metric = new HashSet<>();
		metric.add("temperature");
		metric.add("humidity");
		queryRequest.setSensorIds(ids);
		queryRequest.setMetrics(metric);
		queryRequest.setStatistic("min");
		queryRequest.setFromDate(new Date());
		queryRequest.setToDate(new Date());
		return queryRequest;
	}
